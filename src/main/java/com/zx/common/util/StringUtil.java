...
public final class StringUtil {
    ...
    public static String convertListToStringBuilderContainJingHao(List<String> areaNames) {
        ...
        String areaNamesStr = "";
        if(sb.length() > 0){
            areaNamesStr = sb.toString().substring(1);
        }
        return areaNamesStr;
    }

    public static String convertBirth(String birthDay) {
        birthDay = birthDay.replace("年","-");
        birthDay = birthDay.replace("月","-");
        birthDay = birthDay.replace("日","-");
        String[] split = birthDay.split(Contants.LINE_CHAR);
        String year = split[0].trim();
        String month = leftPad(split[1].trim(),'0',2);
        String day = leftPad(split[2].trim(),'0',2);

        return year + Contants.LINE_CHAR + month + Contants.LINE_CHAR + day;
    }
}
